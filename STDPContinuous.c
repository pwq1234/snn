/*
% Coded based on: Masquelier T, Guyonneau R, Thorpe SJ (2008). Competitive STDP-based Spike Pattern Learning.
...
/*  Not a good idea. Tim 12/2007. (*neuron).weight[afferent] += (*neuron).weight[afferent] * (1-(*neuron).weight[afferent]) * (*param).stdp_a_neg * exp(-(spikeTime-lastFiringTime) / (*param).stdp_t_neg); */
	...
	/* Applying memristor model*/
	delta_t = firingTime-epspTime;
	v = (*param).An * exp(delta_t/(*param).stdp_t_neg);
	wp = ((*param).xp -v)/(1-(*param).xp)+1;
	wn = v /(1-(*param).xn);
	g = (*param).An*(exp(-v) - 1);
	f = exp(-(*param)alphan*(v-(*param).xp))*wp;
	i = (*param)a1*v*sinh((*param)b*v);
c
	(*neuron).weight[epspAfferent] -= v / i;
...
void LTP(NEURON *neuron, double firingTime, PARAM *param) {
	...
	for(e=(int)*(*neuron).nEpsp;e>=1;e--) {
		...
		if(!alreadyPotentiated[epspAfferent]) {/* consider only pairs of spikes*/
			...
			/* Applying memristor model*/
			delta_t = firingTime-epspTime;
			v = (*param).Ap * exp(-delta_t/(*param).stdp_t_pos);
			wp = ((*param).xp -v)/(1-(*param).xp)+1;
			wn = v /(1-(*param).xn);
			g = (*param).Ap*(exp(-v) - 1);
			f = exp(-(*param)alphap*(v-(*param).xp))*wp;
			i = (*param)a1*v*sinh((*param)b*v);

			(*neuron).weight[epspAfferent] += v / i;
			(*neuron).weight[epspAfferent] = min(1-(*param).minWeight,(*neuron).weight[epspAfferent]);
